...
def driver():
    ...
    #trying to find a intial guess that is not close to the root and Newton still converges
    #x0=np.array([-2,-2,-2])
    tol = 1e-8
    ...
    t = time.time()
    for j in range(50):
      [xstar,ier,its] =  both(x0,5*10**-2,tol,Nmax)
    elapsed = time.time()-t
    print(xstar)
    print('Combined: the error message reads:',ier) 
    print('Combined: took this many seconds:',elapsed/50)
    print('Combined: number of iterations is:',its)
...
def both(x,tol_sd,tol,Nmax):
    [xstar,gval,ier_sd,its_sd] = SteepestDescent(x,tol_sd,Nmax)
    if ier_sd==1:
        return [xstar,ier_sd,its_sd]
    else:
        [xstar,ier,its]= Newton(xstar,tol,Nmax)
        return [xstar,ier,its+its_sd]
